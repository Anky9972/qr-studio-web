...
export default function NotificationsPage() {
    ...
    const fetchNotifications = async () => {
        try {
            setLoading(true);
            if (activeTab === 'unread') {
                // Fetch unread notifications from NotificationLog
                ...
                // Fetch all notifications history (including read ones from announcements)
                ...
                >
                    <div className="flex items-center gap-2">
                        ...
                        {unreadNotifications.length > 0 && (
                            <span className="px-2 py-0.5 rounded-full bg-electric-cyan/20 text-electric-cyan text-xs font-bold">
            ...
            <Card variant="glass" className="min-h-[400px]">
                <div className="p-6 border-b border-white/5 flex items-center justify-between">
                    <h2 className="text-lg font-semibold text-white">
                        {activeTab === 'unread' ? 'Unread Notifications' : 'Notification History'}
                    </h2>
                    <span className="text-sm text-gray-400">
                        {currentNotifications.length} {currentNotifications.length === 1 ? 'notification' : 'notifications'}
                    </span>
                </div>
                ...
                <div className="divide-y divide-white/5">
                    {loading ? (
                        ...
                        <div className="p-12 text-center text-gray-500">
                            ...
                            <p>{activeTab === 'unread' ? "No unread notifications. You're all caught up!" : "No notification history found."}</p>
                        </div>
                    ) : (
                        currentNotifications.map((notification) => (
                            <div 
                                key={notification.id} 
                                className="p-6 hover:bg-white/5 transition-colors group flex gap-4 cursor-pointer"
                                onClick={() => {
                                    setSelectedNotification(notification)
                                    setShowDetailModal(true)
                                }}
                            >
                                <div className={cn("mt-1 p-2 rounded-lg", getNotificationColor(notification.type))}>
                                ...
                                <div className="flex-1">
                                    ...
                                    {notification.error && (
                                        <p className="text-xs text-red-400 mt-2">Error: {notification.error}</p>
                                    )}
                                </div>
                                {activeTab === 'unread' && (
                                    <Button
            ...
            {showDetailModal && selectedNotification && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setShowDetailModal(false)}>
                    <motion.div 
                        ...
                        className="bg-gray-900 rounded-xl shadow-2xl border border-white/10 max-w-3xl w-full max-h-[85vh] overflow-hidden flex flex-col"
                        onClick={(e) => e.stopPropagation()}
                    >
                        {/* Header */}
                        <div className="p-6 border-b border-white/10 flex items-start justify-between">
                            <div className="flex items-start gap-4 flex-1">
                                <div className={cn('mt-1 p-3 rounded-xl', getNotificationColor(selectedNotification.type))}>
                                    {getNotificationIcon(selectedNotification.type)}
                                </div>
                                <div className="flex-1">
                                    <h2 className="text-2xl font-bold text-white">{selectedNotification.subject}</h2>
                        ...
                        <div className="flex-1 overflow-y-auto p-6">
                            ...
                            {selectedNotification.error && (
                                <div className="mt-6 p-4 bg-red-900/20 border border-red-800/30 rounded-lg">
                                    ...
                                    <p className="text-sm text-red-300 mt-2">{selectedNotification.error}</p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="p-6 border-t border-white/10 flex justify-between items-center">
                            <span className="text-sm text-gray-400">
                                Type: <span className="text-electric-cyan font-medium capitalize">{selectedNotification.type.replace('_', ' ')}</span>
                            </span>
                            <div className="flex gap-3">
                                {activeTab === 'unread' && (
                                    <Button
