...
export default function NotificationDropdown({ className }: NotificationDropdownProps) {
  ...
  const getNotificationColor = (type: string) => {
    switch (type) {
      case 'announcement': case 'admin_message': return 'text-electric-cyan'
      case 'scan_alert': return 'text-blue-400'
      case 'weekly_report': return 'text-purple-400'
      case 'limit_warning': return 'text-orange-400'
      case 'expiration_reminder': return 'text-yellow-400'
      case 'domain_verified': return 'text-green-400'
  ...
  return (
    <div ref={dropdownRef} className={cn('relative', className)}>
      <button onClick={() => setIsOpen(!isOpen)} className="relative w-10 h-10 rounded-lg flex items-center justify-center text-gray-400 hover:bg-white/5 transition-colors">
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-96 bg-zinc-900 rounded-xl shadow-xl border border-zinc-800 overflow-hidden z-50">
          <div className="p-4 border-b border-zinc-800 flex items-center justify-between">
            <div><h3 className="font-semibold text-white">Notifications</h3><p className="text-xs text-gray-400">{unreadCount > 0 ? `${unreadCount} unread` : 'All caught up!'}</p></div>
            {notifications.length > 0 && <button onClick={markAllAsRead} className="text-xs text-blue-400 hover:underline">Clear all</button>}
          </div>
          <div className="max-h-96 overflow-y-auto">
            {loading ? (<div className="p-8 text-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto"></div><p className="mt-2 text-sm text-gray-400">Loading...</p></div>
            ) : notifications.length === 0 ? (<div className="p-8 text-center"><Bell className="w-12 h-12 mx-auto text-gray-600 mb-2" /><p className="text-sm text-gray-400">No notifications yet</p></div>
            ) : (<div className="divide-y divide-zinc-800">
              {notifications.map((notification) => (
                <div key={notification.id} className="p-4 hover:bg-white/5 transition-colors group cursor-pointer" onClick={() => { setSelectedNotification(notification); setShowDetailModal(true); setIsOpen(false) }}>
                  <div className="flex items-start gap-3">
                    <div className={cn('mt-0.5', getNotificationColor(notification.type))}>{getNotificationIcon(notification.type)}</div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-white mb-1">{notification.subject}</p>
                      <p className="text-xs text-gray-400 mb-1">{formatDistanceToNow(new Date(notification.sentAt), { addSuffix: true })}</p>
                      {notification.error && <p className="text-xs text-red-400">Error: {notification.error}</p>}
                    </div>
                    <button onClick={(e) => { e.stopPropagation(); markAsRead(notification.id) }} className="opacity-0 group-hover:opacity-100 transition-opacity p-1 hover:bg-white/10 rounded" title="Mark as read"><X className="w-4 h-4 text-gray-400" /></button>
                  </div>
                </div>
              ))}
            </div>)}
          </div>
          <div className="p-3 border-t border-zinc-800 bg-zinc-800/50"><button onClick={() => { setIsOpen(false); router.push('/dashboard/notifications') }} className="w-full text-sm text-center text-electric-cyan hover:text-electric-blue transition-colors font-medium">View all notifications</button></div>
        </div>
      )}
      ...
      {showDetailModal && selectedNotification && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setShowDetailModal(false)}>
          <div className="bg-zinc-900 rounded-xl shadow-2xl border border-zinc-800 max-w-2xl w-full max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-zinc-800 flex items-start justify-between">
              <div className="flex items-start gap-3 flex-1"><div className={cn('mt-1 p-2 rounded-lg bg-opacity-10', getNotificationColor(selectedNotification.type))}>{getNotificationIcon(selectedNotification.type)}</div><div className="flex-1"><h2 className="text-xl font-semibold text-white">{selectedNotification.subject}</h2><p className="text-sm text-gray-400 mt-1 flex items-center gap-1"><Clock className="w-3 h-3" />{formatDistanceToNow(new Date(selectedNotification.sentAt), { addSuffix: true })}</p></div></div>
